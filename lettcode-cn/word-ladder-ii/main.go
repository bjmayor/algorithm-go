...
func findLadders(beginWord string, endWord string, wordList []string) [][]string {
	...
	var start = -1
	var end = -1
	var all = make([][]string, 0, 0)
	v := make([]string, 0, len(wordList))
	for i := 0; i < len(wordList); i++ {
		...
		v = append(v, wordList[i])
	}
	if start == -1 {
		v = append(v, beginWord)
		start = len(v) - 1
	}
	...
	g := Graph{}
	g.setv(v)
	size := findonePath(&g, start, end)
	if size == 0 {
		return all
	}
	allpath := findAllPath(&g, start, end, size)

	for _, path := range allpath {
		all = append(all, pathToReadable(path, v))
	}
	return all
}

func pathToReadable(path []int, v []string) []string {
	result := make([]string, 0, len(path))
	for i := len(path) - 1; i >= 0; i-- {
		result = append(result, v[path[i]])
	}
	return result
}

func visited(e Edge, v int) bool {
	if e.from == v {
		return true
	}
	for e.parent != nil {
...
func add2result(v0, vt int, e *Edge, result map[int][][]int) int {
	path := make([]int, 0)
	path = append(path, vt)
	for e != nil {
...
func findonePath(g *Graph, v0 int, vt int) int {
	patharc := make([]int, g.numVertexes, g.numVertexes)
	final := make([]bool, g.numVertexes, g.numVertexes)
	shortest := make([]int, g.numVertexes, g.numVertexes)
	for w := 0; w < g.numVertexes; w++ {
		shortest[w] = g.arc[v0][w]
		patharc[w] = v0
	}
	patharc[v0] = 0
	final[v0] = true
	...
	for i := 1; i < g.numVertexes; i++ {
		...
		fedge := g.edges[k]
		for fedge != nil {
			if !final[fedge.to] && g.arc[k][fedge.to] == 1 && min+1 < shortest[fedge.to] {
				shortest[fedge.to] = min + 1
				patharc[fedge.to] = k
			}
			fedge = fedge.next
		}
...
func findAllPath(g *Graph, v0 int, vt int, min int) [][]int {
	...
	for fedge != nil {
		shortest[fedge.to] = 1
		if fedge.to == vt {
			nodes := add2result(v0, vt, nil, result)
			if nodes < min {
				min = nodes
			}
			break
	...
	for !queue.Empty() {
		...
		fedge := g.edges[e.to]
		for fedge != nil {
			if !visited(e, fedge.to) {

				if (shortest[e.to]+1 <= shortest[fedge.to] || shortest[fedge.to] == 0) && shortest[e.to]+1 <= min {
					shortest[fedge.to] = shortest[e.to] + 1
					if fedge.to == vt && shortest[fedge.to] <= min {
						add2result(v0, vt, &e, result)
						break
					}
					//选择顶点
					queue.push(Edge{
...
type Edge struct {
	from   int
	to     int
	parent *Edge //用户找最短路径是，判断是否已经在路径中
	next   *Edge //查from顶点的所有边
}
...
func (g *Graph) setv(v []string) {
	g.numVertexes = len(v)
	g.edges = make([]*Edge, g.numVertexes, g.numVertexes)
	g.arc = make([][]int, g.numVertexes, g.numVertexes)
	g.v = v
	for i := 0; i < len(v); i++ {
		first := true
		var pEdge, cEdge *Edge
		g.arc[i] = make([]int, g.numVertexes, g.numVertexes)
		for j := 0; j < len(v); j++ {
			if i != j && canConnect(v[i], v[j]) {
				...
				g.arc[i][j] = math.MaxInt32
			}
		}
	}
}

func canConnect(w1, w2 string) bool {
	var diff int
	for i := 0; i < len(w1); i++ {
		if w1[i] != w2[i] {
			diff++
...
func main() {
	...
	for _, input := range inputs {
		fmt.Println("------------")
		for _, v := range findLadders(input.beginWord, input.endWord, input.wordList) {
